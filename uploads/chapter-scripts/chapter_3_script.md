...
<body>
    ...
    <script>
        ...
        const scenes = [
  {"title": "Unveiling Motion", "text": "Ever wondered how objects move? Welcome to 'Motion in a Straight Line,' your gateway to kinematics! We'll describe motion precisely, without worrying about what causes it."},
  {"title": "Position, Displacement", "text": "First, let's define where an object is with its position relative to a reference point. Then, we'll explore displacement – the shortest vector path from start to finish, telling us both distance and direction."},
  {"title": "Velocity Concepts", "text": "How fast an object moves is its speed, but velocity adds direction! We'll distinguish between average velocity over a time interval and instantaneous velocity, the velocity at a specific moment."},
  {"title": "Acceleration Defined", "text": "What happens when velocity itself changes? That's acceleration! It's the rate at which an object's velocity changes, crucial for understanding speeding up, slowing down, or changing direction."},
  {"title": "Motion Equations", "text": "Finally, we'll unlock the powerful equations of motion that mathematically relate displacement, velocity, acceleration, and time. Master these to solve any problem involving constant acceleration in a straight line!"}
];
        let isPlaying = false;
