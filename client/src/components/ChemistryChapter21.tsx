...
export function ChemistryChapter21() {
  ...
  const { data: dbQuestions, isLoading: questionsLoading } = useQuery<Question[]>({
    queryKey: ["/api/questions", "topicId", "2106"],
    queryFn: async () => {
      const response = await fetch("/api/questions?topicId=2106");
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="practice" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Attempt Alcohols, Phenols and Ethers practice in NEET-style MCQ format. Submit when you are ready to reveal the solutions.
              </p>
              <div className="flex flex-col gap-3 md:flex-row">
                {!showSolutions ? (
                  <Button
                    onClick={checkAnswers}
                    disabled={practiceQuestions.length === 0 || Object.keys(userAnswers).length === 0}
                    className="flex-1"
                  >
                    Check Answers
                  </Button>
                    ...
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div>
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <Badge variant="secondary">Level {q.difficultyLevel}</Badge>
                        </div>
                        <p className="font-medium mt-2">{q.questionText}</p>
                      </div>
                    </div>
                    <div className="space-y-2">
