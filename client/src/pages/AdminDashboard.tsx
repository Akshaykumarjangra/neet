...
export default function AdminDashboard() {
  ...
  const [perfRecomputeLoading, setPerfRecomputeLoading] = useState(false);
  const { data: perfMeta, refetch: refetchPerfMeta } = useQuery<{ lastUpdated: string | null; userCount: number }>({
    queryKey: ["/api/analytics/performance-summary/meta"],
    queryFn: async () => {
      return apiRequest("GET", "/api/analytics/performance-summary/meta");
    },
    staleTime: 5 * 60 * 1000,
  });
  const { data: perfTable = [], isLoading: perfTableLoading } = useQuery<any[]>({
    queryKey: ["/api/analytics/performance-summary"],
    queryFn: async () => apiRequest("GET", "/api/analytics/performance-summary"),
    staleTime: 5 * 60 * 1000,
  });

  useEffect(() => {
    if (user && !user.isAdmin && user.role !== 'admin' && user.role !== 'mentor') {
      setLocation("/");
    }
  }, [user, setLocation]);
  ...
  const handleRecomputePerformance = async () => {
    try {
      setPerfRecomputeLoading(true);
      await apiRequest("POST", "/api/analytics/recompute-performance-summary");
      refetchPerfMeta();
      toast({
        title: "Performance summary refreshed",
        description: "Aggregated stats updated for all users.",
      });
    } catch (err: any) {
      toast({
        title: "Refresh failed",
        description: err?.message || "Could not refresh performance summary.",
        variant: "destructive",
      });
    } finally {
      setPerfRecomputeLoading(false);
    }
  };

  // ALL hooks MUST be above early returns (React rules of hooks)
  const isAuthorized = !!user && (user.isAdmin || user.role === 'admin' || user.role === 'mentor');
  ...
  const stats = {
    ...
    paidUsers: userList.filter(u => u.isPaidUser).length,
    adminUsers: userList.filter(u => u.isAdmin).length,
    totalPoints: userList.reduce((sum, u) => sum + (u.totalPoints || 0), 0),
  ...
  const navigationCards = [
    ...
    },
    {
      ...
      description: "Manage test series & exam schedules",
      icon: Calendar,
      color: "text-red-500",
    ...
    },
    {
      ...
      description: "Review pending content updates",
      icon: ShieldCheck,
      ...
      <main className="gradient-mesh-bg p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
            ...
            <Card className="glass-panel col-span-2 md:col-span-3 lg:col-span-6" data-testid="stat-refresh-performance">
              <CardContent className="pt-4 flex items-center justify-between gap-4">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    Refresh aggregated attempts/accuracy for dashboards.
                    <div className="text-xs text-muted-foreground mt-1">
                      Last updated: {perfMeta?.lastUpdated ? new Date(perfMeta.lastUpdated).toLocaleString() : "—"} · Users tracked: {perfMeta?.userCount ?? 0}
                    </div>
                  </div>
                </div>
                <Button
                  variant="outline"
                  onClick={handleRecomputePerformance}
                  disabled={perfRecomputeLoading}
                  data-testid="button-refresh-performance-summary"
                >
                  {perfRecomputeLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <RefreshCw className="h-4 w-4 mr-2" />}
                  Refresh summary
                </Button>
              </CardContent>
            </Card>
          ...
          <Card className="glass-panel mb-8" data-testid="recent-activity-section">
            ...
            <CardContent>
              <div className="space-y-4">
                {userList.slice(0, 5).map((u, index) => (
            ...
            <Card className="glass-panel mb-8" data-testid="user-management-section">
              ...
              <CollapsibleContent>
                <CardContent className="pt-0">
                  ...
                  {/* Users List */}
                  <div className="space-y-2">
                    <h4 className="font-medium">All Users ({userList.length})</h4>
                    {isLoading ? (
                      <p>Loading users...</p>
                    ) : (
                      <div className="max-h-[400px] overflow-y-auto space-y-2">
                        {userList.map((u) => (
                          <div
                            key={u.id}
                            className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between p-4 rounded-lg bg-card/50 hover:bg-muted/50 transition-colors"
