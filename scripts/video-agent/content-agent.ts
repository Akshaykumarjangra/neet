...
import { fetchChaptersWithoutScripts, saveScriptAsset, closePool } from "./db-content-ops.js";
import dotenv from "dotenv";
dotenv.config();

function getHtmlTemplate(scenesJsonStr: string): string {
  // Use the beautifully designed Obsidian Lab HTML template, but inject the dynamic scenes
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
async function generateScriptWithGemini(chapter: any): Promise<string> {
  const { geminiApiKey, geminiModel } = VIDEO_AGENT_CONFIG;

  const systemPrompt = `You are an expert AI content creator and instructional designer for NEET preparation.
Your task is to create a highly engaging, structured script optimized for a visual Explainer Video.

You MUST output ONLY a pure JSON array containing exactly 5 scenes. Do not use markdown code blocks like \`\`\`json. Just raw JSON array.
Each scene object in the array must have exactly two keys:
- "title": A short, 2-3 word title for the scene (e.g. "Cellular Division")
- "text": The conversational voiceover text that the AI will narrate (about 2-3 sentences max).

Example output:
[
  {"title": "The Living World", "text": "Welcome to Chapter 1! Today we explore..."},
  {"title": "Characteristics", "text": "Life has five main traits..."}
]`;

  const userPrompt = `Generate the 5-scene JSON script for the following NEET chapter:

Subject: ${chapter.subject}
Class Level: ${chapter.class_level}
Chapter ${chapter.chapter_number}: ${chapter.chapter_title}
Introduction: ${chapter.introduction?.slice(0, 500)}
Key Concepts: ${Array.isArray(chapter.key_concepts) ? chapter.key_concepts.slice(0, 5).map((k: any) => k.title || k).join(", ") : chapter.key_concepts}`;
  ...
  return getHtmlTemplate(text);
}

async function runContentAgent() {
  console.log("🚀 Starting Agent to Generate HTML 3D Explainers...");
  try {
    const chapters = await fetchChaptersWithoutScripts();
    console.log(`Found ${chapters.length} chapters needing explainers.`);
    ...
    for (let i = 0; i < chapters.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (chapter) => {
        try {
          console.log(`📝 Generating HTML Explainer for Ch${chapter.chapter_number}: ${chapter.chapter_title}...`);
          const htmlContent = await generateScriptWithGemini(chapter);
          
          const assetId = await saveScriptAsset(
            chapter.id, 
            `${chapter.subject} Ch${chapter.chapter_number} Explainer Video`, 
            htmlContent
          );
          console.log(`✅ Saved HTML Explainer to DB! (Asset ID: ${assetId})`);
        } catch (err) {
          console.error(`❌ Failed to generate for Ch${chapter.chapter_number}:`, err);
        }
      }));
      
      console.log(`⏸ Pausing for 5 seconds to avoid rate limits...`);
