...
test.describe('User Journey', () => {
    ...
    test('should load the home page successfully', async ({ page }) => {
        ...
        // Fallback test id check if button-cta-signup is changed
        await expect(page.locator('button').filter({ hasText: /Start Learning/i }).first()).toBeVisible();
    });
    ...
    test('should navigate to pricing', async ({ page }) => {
        ...
        await expect(page.locator('h1, h2').filter({ hasText: /Choose your plan|Pricing/i }).first()).toBeVisible();
    });
