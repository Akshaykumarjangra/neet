...
router.get("/topics", requireAdminOrMentor, async (req, res) => {
  try {
    // ⚡ Bolt Optimization: Replaced N+1 Promise.all(map) count queries
    // with a single batched LEFT JOIN and GROUP BY aggregation.
    // Reduces database queries from (1 + N) to exactly 1 query.
...
router.get("/flashcard-decks", requireAdmin, async (req, res) => {
  try {
    // ⚡ Bolt Optimization: Eliminated N+1 counting queries in map loop
    // by joining flashcards directly and aggregating with GROUP BY.
    // Resolves significant latency bottleneck on dashboard load.
