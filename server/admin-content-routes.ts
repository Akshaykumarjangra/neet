...
router.get("/topics", requireAdminOrMentor, async (req, res) => {
  try {
    // ⚡ Bolt: Removed N+1 queries. Used single grouped query for topics and question counts.
...
router.get("/flashcard-decks", requireAdmin, async (req, res) => {
  try {
    // ⚡ Bolt: Removed N+1 queries. Used single grouped query for flashcard decks and card counts.
