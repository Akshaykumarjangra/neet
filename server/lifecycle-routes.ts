...
router.post('/run-triggers', requireAdmin, async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Fixed N+1 query by using innerJoin to fetch user emails directly instead of querying users in a loop
    const recentHighScores = await db.select({
        userId: mockExamAttempts.userId,
        email: users.email
      })
      .from(mockExamAttempts)
      .innerJoin(users, eq(mockExamAttempts.userId, users.id))
      .where(and(
