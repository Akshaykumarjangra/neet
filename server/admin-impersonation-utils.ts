...
/**
 * Middleware: enforce 1-hour cap on any impersonation session.
 * Mounted globally so any request made under impersonation is rejected once the
...
export function enforceImpersonationTimeLimit(req: Request, res: Response, next: NextFunction) {
    if ((req.session as any)?.originalAdminId && (req.session as any).impersonationStartedAt) {
        const elapsed = Date.now() - (req.session as any).impersonationStartedAt;
