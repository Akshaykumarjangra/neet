...
function detectSillyMistakes(responses: any[], questions: any[]): any[] {
  ...
  // Bolt Optimization: Pre-compute a Map for O(1) lookups instead of O(N) array.find()
  const questionsMap = new Map(questions.map((qq: any) => [qq.id, qq]));
...
router.get('/analyze/:attemptId', requireAuth, async (req, res) => {
  try {
    ...
    // Bolt Optimization: Pre-compute a Map for O(1) lookups instead of O(N) array.find()
    const questionDataMap = new Map(questionData.map(qq => [qq.id, qq]));
    for (const resp of responses) {
