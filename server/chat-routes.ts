...
router.get("/threads", requireAuth, async (req, res) => {
  try {
    ...
    // Optimized: Replaced N+1 query loop with a single batched query using selectDistinctOn
    const threadIds = threads.map(t => t.id);
    let latestMessages: typeof chatMessages.$inferSelect[] = [];
    if (threadIds.length > 0) {
      latestMessages = await db
        .selectDistinctOn([chatMessages.threadId])
        .from(chatMessages)
        .where(inArray(chatMessages.threadId, [...new Set(threadIds)]))
        .orderBy(chatMessages.threadId, desc(chatMessages.createdAt));
    }

    const messagesByThreadId = new Map(latestMessages.map(m => [m.threadId, m]));

    const threadsWithPreview = threads.map(thread => {
      const latest = messagesByThreadId.get(thread.id);
      return {
        ...thread,
