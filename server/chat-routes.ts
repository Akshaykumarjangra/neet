...
router.get("/threads", requireAuth, async (req, res) => {
  try {
    ...
    // ⚡ Bolt Optimization: Fixed N+1 query by batching latest message fetches using selectDistinctOn
    let threadsWithPreview = threads.map(t => ({ ...t, latestMessage: null as any }));
    ...
    if (threads.length > 0) {
      ...
      const messageMap = new Map(latestMessages.map(m => [m.threadId, m]));
